...
print("Iniciando tokenización y lematización...")

for t in titulos:
	doc1= nlp(t)
	for tag in doc1:
		#print(tag)
		tokens_titulos.append((tag.text, tag.tag_, tag.lemma_))
		#lem_titulos.append(tag.lemma_)
print(tokens_titulos[0])
print("Tokenización y lematización de TÍTULOS completada.")

for c in contenido:
	#print(c)
	doc2= nlp(c)
	for tag in doc2:
		tokens_content.append((tag.text, tag.tag_, tag.lemma_))
		#lem_cont.append(tag.lemma_)
print("Tokenización y lematización de NOTICIAS completada.\n")

print("=== Corpus lematizado correctamente ===")

#corpus= np.array([ids, tkn_t, tkn_c], dtype=object).T
corpus= np.column_stack([ids, tokens_titulos[0], tokens_content[0]], dtype=object)
print(corpus[1])
...
datos= pd.DataFrame(corpus, columns=['ID', 'Titulos', 'Noticias'])
...
datos.to_csv('corpus_tokens_titulos.csv', index=False, encoding="latin1")
...
for x in tag_titulos:
	...
	if x[2] not in eliminar:
		...
		tag_titulos.append(x[1])
		lem_titulos.append(x[2])
...
for x in tag_contenido:
	...
	if x[2] not in eliminar:
		#print('y=',y)
		txt_cont.append(x[0])
		tag_contenido.append(x[1])
		lem_cont.append(x[2])

print("Elementos eliminados de las noticias.\n")

print("=== Stop Words eliminadas correctamente ===")
