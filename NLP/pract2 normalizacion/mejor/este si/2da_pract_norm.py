...
corpus= np.array([ids, tokens_content], dtype=object).T
...
datos= pd.DataFrame(corpus, columns=['ID', 'Noticia'])
...
datos.to_csv('tokens_noticias.csv', index=False, encoding="latin1")
...
cont_lemma= np.array([ids, txt_cont], dtype=object).T
...
cont_lemma= pd.DataFrame(cont_lemma, columns=['ID', 'Noticia lematizada'])
...
cont_lemma.to_csv('noticia_lematizado.csv', index=False, encoding="latin1")
...
#~ Almacena las listas en un arreglo 2D en forma de columnas mediante la transpuesta de la matriz
cont_norm= np.array([ids, norm_cont], dtype=object).T
...
#datos= pd.DataFrame(lem_tokens, columns=['ID', 'Token_titulo y etiqueta', 'Token_Contenido y etiqueta'])
cont_norm= pd.DataFrame(cont_norm, columns=['ID', 'Noticia normalizada'])
#c_lem_tokens= pd.DataFrame(c_lem_tokens, columns=['Token_noticia', 'Token_lematizado', 'Etiqueta'])
...
#np.savetxt('noticia.csv', t_lem_tokens, delimiter=",", fmt="%s", encoding="latin1", header="ID, Noticia")
cont_norm.to_csv('noticia_normalizado.csv', index=False, encoding="latin1")
#c_lem_tokens.to_csv('tokens_noticias_lematizados.csv', sep=",", index=False, encoding="latin1")
