# Modelo que utiliza transformers para procesamiento de texto
import pandas as pd
...
labels = data["ideology_binary"].tolist()
...
# Codificar las etiquetas
label_map = {"politician": 0, "celebrity": 1, "journalist": 2} 
train_labels_encoded = [label_map[label] for label in train_labels]
test_labels_encoded = [label_map[label] for label in test_labels]
...
# Cargar el modelo BERT pre-entrenado
model = BertForSequenceClassification.from_pretrained("dccuchile/bert-base-spanish-wwm-uncased", num_labels=3)  # Cambiado a tres etiquetas
...
# Obtener el informe de clasificación
report = classification_report(total_labels, total_predicted, target_names=["politician", "celebrity", "journalist"])
print(report)
...
# Entrenar el modelo usando gpu si esta disponible
if torch.cuda.is_available():
    device = torch.device("cuda")  # Utiliza la GPU
    print(f"GPU: {torch.cuda.get_device_name(0)}")
else:
    device = torch.device("cpu")  # Utiliza la CPU
    print("CPU")
model.to(device)
...
# Obtener el informe de clasificación
report = classification_report(total_labels, total_predicted, target_names=["left", "right"])
print(report)

#-------------IDEOLOGÍA MULTICLASE-----------------
#No son los mejores los mejores parámetros, pero sí son los que dan resultados más equilibrados
# Codificar las etiquetas
...
# Cargar el modelo BERT pre-entrenado
model = BertForSequenceClassification.from_pretrained("dccuchile/bert-base-spanish-wwm-uncased", num_labels=4)  # Cambiado a tres etiquetas
...
# Obtener el informe de clasificación
report = classification_report(total_labels, total_predicted, target_names=["left", "moderate_left", "moderate_right", "right"])
print(report)