...
if __name__=='__main__':
	...
	np.savetxt("data_test_" + str(n_splits) + ".csv", my_data_set.test_set.X_test, delimiter=",", fmt="%s",
	...
	np.savetxt("target_test" + str(n_splits) + ".csv", my_data_set.test_set.y_test, delimiter=",", fmt="%s",
           header="RainTomorrow", comments="")
    
	i = 1
	for val_set in my_data_set.validation_set:
		np.savetxt("data_validation_train_" + str(n_splits) + "_" + str(i) + ".csv", val_set.X_train, delimiter=",", fmt="%s",
		...
		#np.savetxt("data_validation_test" + str(n_splits) + str(i) + ".csv", val_set.X_test, delimiter=",", fmt="%s",
		#header="Date,Location,MinTemp,MaxTemp,Rainfall,Evaporation,Sunshine,WindGustDir,WindGustSpeed,WindDir9am,WindDir3pm,WindSpeed9am,WindSpeed3pm,Humidity9am,Humidity3pm,Pressure9am,Pressure3pm,Cloud9am,Cloud3pm,Temp9am,Temp3pm,RainToday", comments="")
		np.savetxt("target_validation_train_" + str(n_splits) + "_" + str(i) + ".csv", val_set.y_train, delimiter=",", fmt="%s",
		header="RainTomorrow", comments="")
		#np.savetxt("target_validation_test" + str(n_splits) + str(i) + ".csv", val_set.y_test, delimiter=",", fmt="%s",
		#header="RainTomorrow", comments="")
		i = i + 1
